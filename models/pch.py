...
class Net(torch.nn.Module):
    """
    Defines the neural network for PC-Hazard models.
    """
...
def init_model(in_features, hidden_features, labtrans):
    """
    Initializes a Pycox PC-Hazard model.
    """
    return PCHazard(
...
def train_pch(X_train, y_train, X_val, y_val, hidden_features, labtrans, msg=""):
    """
    Trains a PC-Hazard model.
    """
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.model.fit(self.Xc, self.yc, epochs=1, verbose=False, check_out_features=False)
...
def fed_train_pch(eval_fn, in_features, hidden_features, labtrans, client_data, fed_rounds, dataset):
...
def pch_exp(
    ...
    # preprocess labels
    labtrans = PCHazard.label_transform(len(np.unique(y_train["time"])) // 10)
    ...
    glob_model = train_pch(X_train, y_train, X_val, y_val, hidden_features, labtrans,
    ...
    for i, ((Xc_train, yc_train), (Xc_val, yc_val)) in enumerate(zip(client_data, val_client_data)):
        iso_model = train_pch(Xc_train, yc_train, Xc_val, yc_val, hidden_features, labtrans,
    ...
    fed_c_idx, fed_c_idx_ipcw, fed_ibs = \
        fed_train_pch(eval_fn, X_train.shape[1], hidden_features, labtrans, client_data, fed_rounds, dataset_name)
