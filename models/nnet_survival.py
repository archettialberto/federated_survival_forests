...
class Net(torch.nn.Module):
    """
    Defines the neural network for Nnet-survival models.
    """
...
def init_model(in_features, hidden_features, labtrans):
    """
    Initializes a Pycox Nnet-survival model.
    """
...
def train_nnet(X_train, y_train, X_val, y_val, hidden_features, labtrans, msg=""):
    """
    Trains a Nnet-survival model.
    """
...
def fed_train_nnet(eval_fn, in_features, hidden_features, labtrans, client_data, fed_rounds, dataset):
...
def nnet_exp(
    ...
    # preprocess labels
    labtrans = LogisticHazard.label_transform(len(np.unique(y_train["time"])) // 10)
    ...
    glob_model = train_nnet(X_train, y_train, X_val, y_val, hidden_features, labtrans,
    ...
    for i, ((Xc_train, yc_train), (Xc_val, yc_val)) in enumerate(zip(client_data, val_client_data)):
        iso_model = train_nnet(Xc_train, yc_train, Xc_val, yc_val, hidden_features, labtrans,
    ...
    fed_c_idx, fed_c_idx_ipcw, fed_ibs = \
        fed_train_nnet(eval_fn, X_train.shape[1], hidden_features, labtrans, client_data, fed_rounds, dataset_name)
