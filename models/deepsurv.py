...
class Net(torch.nn.Module):
    """
    Defines the neural network for DeepSurv models.
    """
...
def init_model(in_features, hidden_features):
    """
    Initializes a Pycox DeepSurv model.
    """
    return CoxPH(Net(in_features, hidden_features), torch.optim.Adam)


def train_ds(X_train, y_train, X_val, y_val, hidden_features, msg=""):
    """
    Trains a DeepSurv model.
    """
    _y_train = to_pycox_labels(y_train)
    _y_val = to_pycox_labels(y_val)
    val_data = (X_val, _y_val)
    model = init_model(X_train.shape[1], hidden_features)
    callbacks = [tt.callbacks.EarlyStopping(patience=100)]
    log = model.fit(X_train, _y_train, epochs=1000, val_data=val_data, verbose=False, callbacks=callbacks)
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, cid, Xc, yc, hidden_features):
        self.cid = cid
        self.model = init_model(Xc.shape[1], hidden_features)
...
def get_client_fn(client_data, hidden_features):
    """
    ...
    def client_fn(cid) -> FlowerClient:
        ...
        y = to_pycox_labels(y)
        return FlowerClient(cid, X, y, hidden_features)
    return client_fn


def get_evaluate_fn(X_train, y_train, X_val, y_val, X_test, y_test, compute_baseline, hidden_features):
    """
    ...
    def evaluate(server_round, parameters, config):
        ...
        return model.partial_log_likelihood(X_test, _y_test), {
...
def fed_train_ds(eval_fn, in_features, hidden_features, client_data, fed_rounds, dataset):
    params = get_parameters(init_model(in_features, hidden_features))
    ...
    fed_logs = fl.simulation.start_simulation(
        client_fn=get_client_fn(client_data, hidden_features),
...
def ds_exp(
    ...
    eval_fn = get_evaluate_fn(X_train, y_train, X_val, y_val, X_test, y_test, compute_baseline, hidden_features)

    # train/test global model
    print(f"[{datetime.now()}] Training global model...")
    glob_model = train_ds(X_train, y_train, X_val, y_val, hidden_features, msg=f"{dataset_name} - global")
    ...
    for i, ((Xc_train, yc_train), (Xc_val, yc_val)) in enumerate(zip(client_data, val_client_data)):
        iso_model = train_ds(Xc_train, yc_train, Xc_val, yc_val, hidden_features, msg=f"{dataset_name} - client {i}")
    ...
    fed_c_idx, fed_c_idx_ipcw, fed_ibs = \
        fed_train_ds(eval_fn, X_train.shape[1], hidden_features, client_data, fed_rounds, dataset_name)
