...
class Net(torch.nn.Module):
    """
    Defines the neural network for CoxPH models.
    """
    def __init__(self, in_features):
        super().__init__()
        self.output = torch.nn.Linear(in_features=in_features, out_features=1, bias=False)
...
def init_model(in_features):
    """
    Initializes a Pycox CoxPH model.
    """
    return CoxPH(Net(in_features), torch.optim.Adam)


def train_cox(X_train, y_train, X_val, y_val, msg=""):
    """
    Trains a CoxPH model.
    """
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, cid, Xc, yc):
        self.cid = cid
        self.model = init_model(Xc.shape[1])
...
def get_evaluate_fn(X_train, y_train, X_val, y_val, X_test, y_test, compute_baseline):
    """
...
def fed_train_cox(eval_fn, in_features, client_data, fed_rounds, dataset):
    params = get_parameters(init_model(in_features))
    ...
    fed_logs = fl.simulation.start_simulation(
        client_fn=get_client_fn(client_data),
...
def cox_exp(
    ...
    eval_fn = get_evaluate_fn(X_train, y_train, X_val, y_val, X_test, y_test, compute_baseline=True)

    # train/test global model
    print(f"[{datetime.now()}] Training global model...")
    glob_model = train_cox(X_train, y_train, X_val, y_val, msg=f"{dataset_name} - global")
    ...
    for i, ((Xc_train, yc_train), (Xc_val, yc_val)) in enumerate(zip(client_data, val_client_data)):
        iso_model = train_cox(Xc_train, yc_train, Xc_val, yc_val, msg=f"{dataset_name} - client {i}")
    ...
    fed_c_idx, fed_c_idx_ipcw, fed_ibs = fed_train_cox(eval_fn, X_train.shape[1], client_data, fed_rounds, dataset_name)
